...
public class MultiTouchDisplay : MonoBehaviour
{
    ...
    void Update()
    {
        multiTouchInfo = ($"Max Tap Count: { 0}\n, maxTapCount");
        ...
        if(Input.touchCount > 0)
        {
            for(int i = 0; i < Input.touchCount; i++)
            {
                ...
                multiTouchInfo += ($"Touch { 0} -Position { 1} - Tap Count: { 2} -Finger ID: { 3}\n Radius: { 4} ({ 5}%)\n",
                    i, theTouch.position, theTouch.tapCount, theTouch.fingerId, theTouch.radius, (theTouch.radius / (theTouch.radius
